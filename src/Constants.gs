...
// 業務のある曜日。シフト入力の曜日の選択肢と、マイページ・申請画面の空きコマ表が
// **すべてここから導出される**（backlog 10-5）。
//
// かつてシフト入力（土あり）と空きコマ収集（土なし）が別々に曜日を持っていてドリフトし、
// 土曜コマを登録しても誰も available_slots に「土n」を持てず、補充候補が構造的に0人＝
// 毎回「職員対応」に化ける穴があった（D17）。出所を1つにして一致を保証している。
//
