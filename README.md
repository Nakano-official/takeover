...
| | |
|---|---|
| **技術** | Google Apps Script / HTML Service / Spreadsheet / Google Chat Webhook / M5Stack |
...
| **体制** | 個人開発（要件定義・設計・実装・ドキュメント・職員向けマニュアル）。実装は Claude Code を併用 |
...
→ CSV をアップロードすると、**登録漏れ / 予定外勤務 / 時間不一致**を自動でハイライトします。
...
| 端末エンドポイント | `?device=alert` | M5Stack | 未対応欠員の**件数と最新番号のみ**返す（個人情報なし） |
...
- M5Stack 端末のエンドポイントは、トークンで保護したうえで**件数と最新番号しか返さない**
...
| クラウドから LAN 内の端末へ push できない | M5Stack が **GAS を定期ポーリング** | 端末側で「確認した」状態を持てば、GASは読み取り専用で済む |
...
| B | 勤怠CSV照合エンジン（Shift_JIS・15分丸め・差分3型） | ✅ |
| B | 整合性チェック画面（アップロード・期間指定・ハイライト） | ✅ |
| B | 実CSVヘッダーの最終確認・実カレンダーでの表記ゆれ確認 | ⏳ 実データ待ち |
| B | 月末リマインダー・教務課向けレポート出力 | 📋 Phase 3 |
| 共通 | 利用学生中心の時間割・マイビュー・レスポンシブ | ✅ |
| 共通 | シフト入力（追加・編集・削除／候補絞り込み） | ✅ |
| 共通 | フォーム連携（空きコマ・連絡先の自動取り込み） | ✅ |
| 共通 | 学期マスタ・学期設定画面 | ✅ |
| 共通 | M5Stack 物理アラート端末（GASエンドポイント＋スケッチ） | ✅ 実機通電テストは未実施 |

**未検証として残していること**（正直に書いておきます）：実機での同時押しテスト、
GAS 本番環境でのパフォーマンス実測（②は合成シミュレーション36アサートで検証）、
M5Stack の通電・Wi-Fi・通知テスト、実勤怠CSVでの照合。
