...
flowchart TD
  ...
  F --> G["本人・他候補・職員へ自動通知"]
  D -->|0人| J["即・募集クローズ（職員へ決着要求・D22）"]
  D -->|候補はいるが返信が来ない| H["候補の電話番号を職員画面に表示 → 電話フォロー（D11）"]
  H --> K["口頭OKなら職員が手動確定"]
  D -->|承諾なしで締切到達| L["時間トリガーが募集クローズ（D22）"]
  J --> I["職員が決着：相方残→1人テイク／残らず→職員対応（再オープン可）"]
  L --> I
  K --> I
```
...
> ⚠️ **機能B（勤怠整合性チェック）は今回のリリース対象外**（D41）。コードは
> `future/feature-b/` にあり、**本番のGASプロジェクトには存在しない**。今後作る予定は残っている。
...
### 機能B：勤怠整合性チェックの自動化（※ 今回のリリース対象外・D41）
...
| 回答締切・全員辞退時の運用（**決定済み・D21/D22**） | **締切＝授業開始30分前**（`RECRUIT_DEADLINE_MIN_BEFORE`）。締切を過ぎた欠勤連絡は「登録のみ受け付け・代行募集なし・職員へ通知・学生には Classroom への連絡を案内」。締切に到達したら**時間トリガー**（`Triggers.gs`・10分間隔）が**募集をクローズするだけ**で `result` は書かず、職員へ決着を要求する（D22）。D10/D21 にあった自動決着は撤去済み。**未決**：締切**前**のリマインドのタイミング・回数・宛先／夜間・休日の送信可否 |
...
- [x] 先着自動確定（D1）／補充候補0人・締切到達時の募集クローズ・再オープン（D10→D22）
...
- [—] 勤怠整合性チェック ※**今回のリリース対象外**・`future/feature-b/` へ退避（D41）
