...
## 14. 担当が決まる前に授業だけ登録できない　🔴（2026-09-12・本番投入で判明）
...
🔴。毎学期「授業だけ先に入れる」場面が来るうえ、いま本番に嘘のデータが入っている。
学生の登録が済んで担当を付け替えるタイミングまでに直っていると、付け替え作業がそのまま片付く。
...
- ⚪ **12-1. 開発者不在後の「実行者」は誰か（最大の単一障害点）**
  ...
  - その職員の異動・退職時の**引き継ぎ手順（再デプロイ・オーナー移管・スクリプトプロパティの引き継ぎ・
    トリガーの再インストール）**を operations.md に明文化する。
