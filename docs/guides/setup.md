...
| 開発中のデプロイ | **スプレッドシート2つを開ける人なら誰でも可**（D42） |
| **引き継ぎ時の最終デプロイ** | **職員アカウント**（開発者が抜けた後も動かすため） |
| `installTriggers()` | 開発中は誰でも可。**引き継ぎ時は職員アカウントで入れ直す** |
| 開発者の動作確認 | `/dev`（デプロイをテスト） |

Web App は `executeAs: USER_DEPLOYING`＝**最後にデプロイした人として動く**。
つまり条件はただ1つ、**デプロイした人がスプレッドシート2つを開けること**。

2026-09-14 にスプレッドシートとGASプロジェクトを職員アカウントへ移管し、開発者は編集者として
共有されている。したがって**開発者がデプロイしても動く**（D42）。開発中は毎回職員に頼まなくてよい。

> ⚠️ **ただし引き継ぎ時は別**。実行アカウントは最後にデプロイした人のままなので、
> 開発者名義でデプロイしたまま卒業・異動すると、**そのアカウントが停止された時点で
> Web App が止まる**（シートの所有者が職員でも関係ない）。
> 抜ける前に**職員アカウントで最終デプロイ**し、`installTriggers()` も入れ直すこと。
> **[6章](#6-開発者アカウントからの引き継ぎ卒業異動で担当が抜けるとき)を参照。**

> ⚠️ **スプレッドシートを開けない人はデプロイしないこと。** 2026-09-12 に M5Stack 担当が
> デプロイして実行アカウントが入れ替わり、全画面が権限エラーで止まった。原因はこれだった。
