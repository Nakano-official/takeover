...
flowchart TD
  ...
  F --> G["本人・他候補・職員へ自動通知"]
  D -->|0人 / 返信が来ない| H["候補の電話番号を職員画面に表示 → 電話フォロー"]
  H --> I["相方残→1人テイク／残らず→職員対応（自動・手動、再オープン可）"]
```
...
| イレギュラー対応 | 本人 or 職員がカレンダーを手動修正|
| 勤怠登録 | 月末に学生スタッフが大学ポータルで登録 |
| 照合 | 職員・教務課が「カレンダー ↔ 勤怠データ」を**目視で突き合わせ** |

**問題の本質**: カレンダーと勤怠登録が別々に存在し、人間が手動照合している。

#### フロー比較（現状 → 導入後）

**現状（As-Is）** … 手動修正で漏れが多発し、職員・教務課が二重に目視照合

```mermaid
flowchart TD
  A["クォーター開始：カレンダーへ予定を一括登録"] --> B["イレギュラー発生（超過/早退/休講/欠員/代行）"]
  B --> C["本人 or 職員がカレンダーを手動修正"]
  C --> D["修正漏れが多発"]
  D --> E["月末：学生がポータルで勤怠登録"]
  E --> F["職員がカレンダー↔勤怠を目視照合"]
  F --> G["教務課がもう一度目視照合して承認"]
```

**導入後（To-Be）** … 自動照合・差分ハイライト・承認1ボタン（機能B＝Phase 2 の目標）

```mermaid
flowchart TD
  A["カレンダーの勤務予定"] --> C["システムが自動照合"]
  B["勤怠登録データ（CSV等）"] --> C
  C --> D["差分（登録漏れ・時間不一致）を一覧でハイライト"]
  D --> E["月末前に自動リマインド"]
  E --> F["照合済みレポートを教務課と共有 → 承認ボタン1つで完結"]
```

> 機能B（勤怠整合性チェック）は Phase 2 として未実装。上の To-Be は目標フロー。
> 現状は `Calendar.gs` に予定パースの診断機能のみ。
...
|---|---|
| course_id | コマID |
| quarter | クォーター（例：2026-Q3）。月末照合で過去分を参照するため行は削除しない |
| day | 曜日 |
| period | 時限 |
| staff_a_id | 担当スタッフA |
| staff_b_id | 担当スタッフB |
...
| 項目 | 内容 |
|---|---|
| 勤怠データの入手経路 | 大学ポータルから勤怠データをCSV等でエクスポートできるか職員に確認。不可の場合は機能Bの設計を見直す |
| 回答締切・全員辞退時の運用 | 授業開始までに回答がない場合のリマインド・職員へのエスカレーション方法 |
...
- [ ] 候補者への Google Chat 通知（個人スペース Webhook で個別送信）
- [ ] 代行可否の回答画面（responses シートに記録）
- [ ] 職員ダッシュボード（欠員一覧・回答状況）
