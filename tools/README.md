...
node tools/sim-terms.js       # 学期の解決（現在（開講中）の範囲・学期の境目・D16/D36）
```
...
## sim-terms.js（学期の解決・D16/D36）

`Terms.gs` の「いま表示すべき学期の集合」を検証する。クォーター制（先端理工）と
セメスター制（他学部）が同じ暦の上で同時に走るため、前期⊇1Q+2Q／後期⊇3Q+4Q の入れ子になる。

ここが外れると**登録したコマが時間割から丸ごと消える**。エラーは出ないので、職員からは
「登録したのに出ない＝壊れている」としか見えない。実際 2026-09-17、後期開始の数日前に開いたとき、
寄せ先が 4Q になって 3Q のコマが全部消えた（D36）。**学期の境目は毎年必ず来る**ので、
その時期の挙動をここで固定してある。

- 学期の境目（どの学期にも入っていない日）に、直近の学期へ寄せること
- 開講中は 後期＋3Q＋4Q のように内包するクォーターまで含めること
