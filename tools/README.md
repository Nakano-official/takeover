...
node tools/sim-roster.js      # スタッフ名簿（職員が全員を確認する画面・D30）
```
...
`src/*.html` の `<script>` が**存在しないものを呼んでいないか**を調べる。

`check-syntax.js` は構文しか見ないので、未定義の関数呼び出しは素通りする。これは画面を開くまで
分からないうえ、描画関数の中で起きると**画面が「読み込み中…」のまま固まる**（例外は握られず、
見た目には何も出ない）。2026-09-17 に `input.html` で `escapeAttr` が未定義のまま呼ばれ、
実際にこれが起きた。そのとき足したのがこれ。

| 見るもの | 拾える事故 |
|---|---|
| HTML内で呼んでいる関数が、その HTML か既知のグローバルに在るか | ヘルパーの移植忘れ・タイプミス |
| `google.script.run.〇〇()` の 〇〇 が `src/*.gs` / `*.js` に在るか | サーバー関数名のタイプミス・改名漏れ |

構文解析器は使わず、文字列・コメント・**正規表現リテラル**を空白に潰したうえで走査する。
正規表現の扱いを省くと `/'/g` のようなクォートを含む正規表現を文字列の開始と誤認し、
そこから先の解析が丸ごとずれる（作った当日に `terms.html` で全定義を見失った）。

**検出できないこと**：変数の未定義、引数の個数、実行時の型、DOM 要素の有無。
迷ったら通す側に倒してあるので、これが緑でも画面確認は要る。
...
業務ごとの空きコマ（D32）もここで見る。**テイクは available_slots・介助は assist_slots** で
絞ること、`assist_slots` 列がまだ無いDBでは従来の1本へ寄せること（列を素直に読むと
**介助の候補が全員ぶん消え、しかもエラーが出ない**）。
...
- `sim-profile.js` … 本人が変更できるのは電話・Webhook・空きコマ・対応できる業務の4つだけ（D31）。
  payload に `role` や `name` を混ぜても無視されること、`staff_id` が必ず Session から取られること、
  実在しないスロットを弾くこと。加えて **`skills` を全部外した保存を弾く**こと
  （空欄は D9 で「全対応」扱いなので、外したつもりが**全依頼を受ける**側に倒れる）。
...
## sim-roster.js（スタッフ名簿・D30）

`Roster.gs` を検証する。読み取り専用の画面なので、壊れ方は「書き換え」ではなく**見落とし**に出る。

- **職員限定**であること（連絡先を含むため学生には返さない）
- **Webhook URL そのものを返さない**こと（届くかどうかだけ返す）
- 「欠けていること」を取りこぼさないこと（空きコマ未登録・本人未更新・通知先なし・
  連絡先DBの行が無い）。どれも**気づかれないまま動かない**種類の欠落なので、ここが唯一の検出点になる
- **個人コード（`personal_code`）を返さない**こと。機能Bを実装しない方針になり（2026-09-13）
  読む相手がいなくなった。列はシートに残るので、うっかり画面へ戻さないようここで固定する
- 担当コマ数が今の学期範囲だけを数え、終わった単発コマ（D27）を数えないこと
- `slots_updated_at` が Date で読み戻されても壊れないこと

**検出できないこと**：画面の見た目と操作（絞り込み・行を開いたときの詳細）。ブラウザ確認が要る。
